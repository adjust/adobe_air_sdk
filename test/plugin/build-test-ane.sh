...
if [[ -n $1 ]]; then
    echo -e "${RED}[ADJUST][BUILD-TEST-ANE]:${GREEN} Invalid argument! Aborting ... ${NC}"
    exit 0
fi
...
if [ "${DISPLAY_HELP}" == YES ]; then
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} build-test-ane.sh script is used to build Adjust SDK ANE used for integration tests. ${NC}"
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Arguments: ${NC}"
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN}     --skip-android: Skips building native Android test library ${NC}"
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN}     --skip-ios: Skips building native iOS test library ${NC}"
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN}     --help: Displays help instructions"
    exit 1
fi
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Running emulator tasks ... ${NC}"
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Done! ${NC}"

# ======================================== #

if [ "$BUILD_ANDROID" = YES ]; then
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Cleaning up Android binary files from previous build ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
    ...
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Running Android test library build script ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
    android/build.sh
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Done! ${NC}"
    ...
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Copying generated libraries to their destination (${ROOT_DIR}/${TEST_PLUGIN_DIR}/${BUILD_DIR}/Android) ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
...
if [ "$BUILD_IOS" = YES ]; then
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Cleaning up iOS binary files from previous build ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
    ...
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Running iOS test library build script ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
    ios/build.sh
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Done! ${NC}"
    ...
    echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Copying generated libraries to destination (${ROOT_DIR}/${TEST_PLUGIN_DIR}/${BUILD_DIR}/iOS) ... ${NC}"
    cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Making SWC file ... ${NC}"
cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Copying files to ${BUILD_DIR} directory ... ${NC}"
cd ${ROOT_DIR}/${TEST_PLUGIN_DIR}
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Running ADT ... ${NC}"
...
echo -e "${CYAN}[ADJUST][BUILD-TEST-ANE]:${GREEN} Script completed! ${NC}"
